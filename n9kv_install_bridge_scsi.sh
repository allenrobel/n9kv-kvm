...
virt-install \
    --name="n9kv-S1" \
    --ram=8192 --vcpus=4 \
    --disk path="$N9KV_IMAGE_S1",format=qcow2,bus=scsi,cache=writethrough \
    --network bridge=ndfc-mgmt,model=e1000 \
    --network bridge=BR_ER_S1,model=e1000 \
...
virt-install \
    --name="n9kv-S2" \
    --ram=8192 --vcpus=4 \
    --disk path="$N9KV_IMAGE_S2",format=qcow2,bus=scsi,cache=writethrough \
    --network bridge=ndfc-mgmt,model=e1000 \
    --network bridge=BR_ER_S2,model=e1000 \
...
virt-install \
    --name="n9kv-L1" \
    --ram=8192 --vcpus=4 \
    --disk path="$N9KV_IMAGE_L1",format=qcow2,bus=scsi,cache=writethrough \
    --network bridge=ndfc-mgmt,model=e1000 \
    --network bridge=BR_S1_L1,model=e1000 \
    --network bridge=BR_S2_L1,model=e1000 \
