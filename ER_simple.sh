...
# Simplified N9KV QEMU startup - closer to your working example
IMAGE_PATH="/iso/nxos"
...
if [ ! -f "$BIOS_FILE" ]; then
    ...
    echo "Then copy: cp /usr/share/ovmf/OVMF.fd bios.bin"
    exit 1
fi

ER_IMAGE="$IMAGE_PATH/ER.qcow2"
# Create disk copies (N9KV needs writable disks)
...
# Start ER (Edge Router) - simplified configuration
...
echo "Console access: telnet localhost 9020"
echo ""
echo "If it boots to UEFI shell, try these commands:"
echo "Shell> fs0:"
echo "FS0:> ls"
echo "FS0:> EFI\\BOOT\\BOOTX64.EFI"
echo ""
echo "Or try booting from the qcow2 directly:"
echo "Shell> exit"
echo "# This should take you to boot manager"
echo ""
echo "To stop: pkill -f 'qemu-system.*n9kv-ER'"