...
### Task 5: Add the WAN/ISN bridges to the bridges files

**Files:**

- Modify: `config/bridges/bridges_config_ovs.sh:22-35` (the `BRIDGES` array)
- Modify: `config/bridges/netplan/9912-bridges.yaml` (OVS netplan variant — header table + two bridge stanzas)
- Modify: `config/bridges/9912-bridges.yaml` (Linux-bridge netplan variant — header table + two bridge stanzas)
- Modify: `config/bridges/bridge.conf` (QEMU bridge-helper allowlist)

**Interfaces:**

- Consumes: bridge names `BR_ISN_WAN_S1_1` and `BR_ISN_WAN_S2_1` as referenced by `config/8000v/WAN1.yaml` (Task 1). Both are exactly 15 chars —
  the IFNAMSIZ limit — so any future rename must stay ≤15.
- Produces: host bridges that `8000v.py` attaches TAPs to. The BG-side endpoints (`S1_BG1_INTERFACE_3` / `S2_BG1_INTERFACE_3`) are documented as
  intended wiring only; actually attaching the BGs is a separate change to `config/nexus9000v/S1_BG1.yaml` / `S2_BG1.yaml`, out of scope here.

- [ ] **Step 1: Add the bridges to `bridges_config_ovs.sh`**

In the `BRIDGES=(` array, after the (indented) line `BR_S2_LE1_H1_1`, insert at the same four-space indentation:
...
- [ ] **Step 2: Add the bridges to `config/bridges/netplan/9912-bridges.yaml`**

In the header comment table, after the line `# S1_LE2_INTERFACE_3      S1_TOR1_INTERFACE_2     BR_S1_LE2_T1_1`, insert:

```text
# WAN1_INTERFACE_2        S1_BG1_INTERFACE_3      BR_ISN_WAN_S1_1   (C8000V WAN/ISN; BG side not yet wired)
# WAN1_INTERFACE_3        S2_BG1_INTERFACE_3      BR_ISN_WAN_S2_1   (C8000V WAN/ISN; BG side not yet wired)
```

In the `bridges:` section, after the `BR_S1_LE2_T1_1:` stanza (before `BR_ND_DATA_12:`), insert (matching this file's stanza style, which includes
`openvswitch: {}`):
...
Match the exact `openvswitch: {}` placement used by the neighboring stanzas in that file (it appears as the first key of each bridge stanza there).

- [ ] **Step 3: Add the bridges to `config/bridges/9912-bridges.yaml`**

Same two header-table comment lines as Step 2, inserted at the same spot. In the `bridges:` section, after the `BR_S1_LE2_T1_1:` stanza (before
`BR_ND_DATA_12:`), insert the same two stanzas but WITHOUT the `openvswitch: {}` key (this file is the Linux-bridge netplan variant):
...
- [ ] **Step 4: Add the bridges to `config/bridges/bridge.conf`**

After the line `allow BR_S1_LE2_T1_1`, insert:

```text
# Bridges for WAN1 (C8000V WAN/ISN router)
allow BR_ISN_WAN_S1_1
allow BR_ISN_WAN_S2_1
```
...
```bash
.venv/bin/python -c "import yaml; [yaml.safe_load(open(f)) for f in ('config/bridges/9912-bridges.yaml', 'config/bridges/netplan/9912-bridges.yaml')]; print('YAML OK')"
bash -n config/bridges/bridges_config_ovs.sh && echo "shell OK"
grep -c "BR_ISN_WAN" config/bridges/bridges_config_ovs.sh config/bridges/9912-bridges.yaml config/bridges/netplan/9912-bridges.yaml config/bridges/bridge.conf
```

Expected: `YAML OK`, `shell OK`, then per-file line counts `2`, `4`, `4`, `2` (each netplan YAML has 2 header-comment lines + 2 stanza keys).

- [ ] **Step 6: Commit**

```bash
git add config/bridges/bridges_config_ovs.sh config/bridges/9912-bridges.yaml config/bridges/netplan/9912-bridges.yaml config/bridges/bridge.conf
git commit -m "Add BR_ISN_WAN_S1_1 / BR_ISN_WAN_S2_1 bridges for the C8000V WAN router"
```

---

### Task 6: Wire the border gateways to the WAN bridges

**Files:**

- Modify: `config/nexus9000v/S1_BG1.yaml`
- Modify: `config/nexus9000v/S2_BG1.yaml`

**Interfaces:**

- Consumes: bridges `BR_ISN_WAN_S1_1` / `BR_ISN_WAN_S2_1` (Task 5); n9kv YAML schema where `neighbors` and `isl_bridges` are paired lists and
  zero-based entry `isl_bridges[i]` maps to `Ethernet1/(i+1)`, so the appended third entry becomes `Ethernet1/3`.
- Produces: BG-side attachment to the WAN bridges. **Operational note (do not automate here):** the running S1_BG1/S2_BG1 VMs and their day-0
  ISOs (`config/nexus9000v/startup_config.py`) must be rebuilt/relaunched by the operator to pick up the new NIC; this task only edits YAML.

- [ ] **Step 1: Append the WAN link to `config/nexus9000v/S1_BG1.yaml`**
...
Change those two lists to (appending one entry to each, keeping them paired — `WAN1` becomes `Ethernet1/3`):
...
- [ ] **Step 2: Append the WAN link to `config/nexus9000v/S2_BG1.yaml`**
...
- [ ] **Step 3: Verify both YAMLs still satisfy the launcher's paired-list constraint and render three interfaces**

Run:

```bash
cd config/nexus9000v && ../../.venv/bin/python nexus9000v.py --config S1_BG1.yaml --dry-run | grep -E "ISL_BRIDGE|tap" ; cd ../..
env IOSXE_PASSWORD=x NXOS_PASSWORD=TestPassword .venv/bin/python config/nexus9000v/startup_config.py --print S1_BG1.yaml | grep -A1 "interface Ethernet"
```

Expected: the dry-run lists three ISL interfaces (`ISL_BRIDGE_3` on `BR_ISN_WAN_S1_1`, tap `tap1301-3`), and the rendered startup-config contains
`interface Ethernet1/1`, `Ethernet1/2`, and `Ethernet1/3`, each followed by `no shutdown`. Repeat the dry-run for `S2_BG1.yaml` and expect
`ISL_BRIDGE_3` on `BR_ISN_WAN_S2_1` (tap `tap2301-3`).

- [ ] **Step 4: Commit**

```bash
git add config/nexus9000v/S1_BG1.yaml config/nexus9000v/S2_BG1.yaml
git commit -m "Wire S1_BG1/S2_BG1 to the C8000V WAN bridges (Ethernet1/3)"
```
