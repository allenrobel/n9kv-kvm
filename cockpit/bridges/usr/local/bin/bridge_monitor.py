    ...
    def scan_bridges(self) -> Dict:
        ...
        for bridge_name in bridges:
            try:
                ...
                self.logger.info(f"Collected stats for bridge: {bridge_name}")
            except Exception as e:
                self.logger.error(f"Error collecting stats for bridge {bridge_name}: {e}")
        
        result = {
            "timestamp": datetime.now(timezone.utc).isoformat(),
            "bridge_count": len(bridge_stats),
...
def print_table(data: Dict):
    ...
    for bridge in bridges:
        ...
        print(f"{bridge['name']:<15} {bridge['status']:<8} {rx_bytes_str:<12} {tx_bytes_str:<12} "
