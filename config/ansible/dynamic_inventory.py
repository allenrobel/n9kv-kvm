...
Inspect the resolved inventory (it prints JSON):

```bash
python3 $HOME/repos/n9kv-kvm/config/ansible/dynamic_inventory.py
```

Or pass it to any `ansible-playbook` run with `-i`. The former `cisco.dcnm`
fabric playbooks have been removed (fabric config now runs through Nexus
Dashboard from a separate `ansible-nd` repo); this inventory is retained as the
canonical, env-var-driven source of per-switch IPs and interfaces. NX-OS
startup-config / boot ISOs are generated by
`config/nexus9000v/startup_config.py`, not Ansible.
...
# S1_TOR1_INTERFACE_3     S1_H1_INTERFACE_1       BR_S1_T1_H1_1
...
S1_TOR1_INTERFACE_3 = environ.get("S1_TOR1_INTERFACE_3", "Ethernet1/3")
...
output = {
    ...
    "all": {
        ...
        "vars": {
            ...
            "S1_TOR1_INTERFACE_3": S1_TOR1_INTERFACE_3,
